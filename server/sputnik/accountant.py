...
class Accountant:
    ...
    def post_transaction(self, username, transaction):
        ...
        def notify_fill(result):
            ...
            self.messenger.send_message(user, 'Order fill notification', 'fill', 'fill',
                                        contract=contract, id=order, quantity=quantity, quantity_fmt=util.quantity_fmt(contract, quantity),
                                        price=price, price_fmt=util.price_fmt(contract, price), side=side, timestamp=util.timestamp_to_dt(timestamp))
